...
public class ScriptingEngine {
    ...
    public void loadScript(String filename)
    {
        try
        {
            LoadState.load( new FileInputStream(filename), filename, luaGlobals ).call();
        }
        catch (IOException e)
        {
     ...
     * Beware - Varargs is weird and when dealing with coroutine.yield(a1, a2...)
     * a1 will be found under args.checkvalue(2), a2 with 3, so forth. NIL with 1.
     ...
     * Remember that Java autoboxing allows
     * Object[] o = new Object[] { 1, 'a', "b" };
    ...
    /**
     * Calls the function referenced by functionName registered in the Lua global
    ...
    /**
     * Creates a thread fromthe function referenced by functionName in the Lua global
     * @param functionName the name of the function registered in the Lua global
     * @return the LuaThread created from the provided function
     */
    public LuaThread createThread(String functionName)
    {
        return new LuaThread(luaGlobals.checkglobals(), getLuaFunction(functionName));
    }

    /**
     * Obtains from the Lua global the function referenced by functionName
     * @param functionName the name of the function registered in the Lua global
     * @return the LuaFunction which represents the function referred to by functionName
     */
